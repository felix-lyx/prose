...
class DataOperatorModel(nn.Module):
    ...
    def __init__(
        ...
        # output layer
        if self.with_output:
            # self.proj = nn.Linear(self.dim, params.max_output_dimension * params.x_grid_size, bias=True)
            proj1 = nn.Linear(self.dim, self.hidden_dim, bias=True)
            proj2 = nn.Linear(self.hidden_dim, params.max_output_dimension * params.x_grid_size, bias=True)
            self.proj = nn.Sequential(proj1, proj2)
        self.split_fused_feature = params.split_fused_feature_data
    ...
    def fwd(
        ...
        return tensor  # (slen, bs, output_dim)

    def predict(self, tensor, pred_mask, y, weight=None):  # norms = None):
        """
        Given the last hidden state, compute word scores and/or the loss.
        Inputs:
            tensor     (slen, bs, output_dim)
            pred_mask  (slen, bs, output_dim) or (slen, bs, x_grid_size, output_dim) mask for different dimension/length
            y          (pred_mask.sum(), ) labels for prediction
            weight     (pred_mask.sum(), ) weight for loss function
            norms      (bs,)
        """
        if pred_mask.dim() == 4:
            tensor = tensor.reshape_as(pred_mask)
        # if norms != None:
        #     eps = 1e-6
        #     for i in range(scores.shape[1]):
        #         scores[:, i, ...] *= norms[i] + eps

        scores = tensor[pred_mask]
    ...
    def generate(
        ...
        # tensor = tensor.float()  # (slen, bs, output_dim)
        #
        # if norms != None:
        #     eps = 1e-6
        #     for i in range(tensor.shape[1]):
        #         tensor[:, i, ...] *= norms[i] + eps
        return tensor
...
class RevIN(nn.Module):
    ...
    def __init__(
        ...
        """
        Reversible instance normalization for data
        """
        super(RevIN, self).__init__()
        self.gamma = nn.Parameter(torch.ones(params.max_output_dimension * params.x_patch_size))
        self.beta = nn.Parameter(torch.zeros(params.max_output_dimension * params.x_patch_size))

    def forward(self, x, eps=1e-6):
        mu_T = x.mean(dim=0, keepdim=True)[:, :, 1:]
        var_T = x.var(dim=0, keepdim=True, unbiased=False)[:, :, 1:]
        x_hat = (x[:, :, 1:] - mu_T) / (var_T + eps).sqrt()
        x_transformed = self.gamma * x_hat + self.beta
        x_transform = x.clone()
        x_transform[:, :, 1:] = x_transformed
        return x_transform, mu_T, var_T

    def reverse(self, x_transformed, mu_T, var_T, eps=1e-6):
        x_hat = (x_transformed - self.beta) / self.gamma
        x = x_hat * (var_T + eps).sqrt() + mu_T
        return x
