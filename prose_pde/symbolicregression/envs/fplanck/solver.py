...
class fokker_planck:
    def __init__(self, *, temperature, drag, extent, resolution,
        ...
        axes = [np.arange(self.Ngrid[i])*self.resolution[i]+ .5 *self.resolution[i] for i in range(self.ndim)]
        # for axis in axes:
    ...
    def propagate_interval(self, p0, tf, Nsteps=None, dt=None, normalize=True):
