...
class Trainer(object):
    def __init__(self, modules, env, params, path=None, root=None):
        ...
        if params.export_data:
            ...
            params.export_path_prefix = os.path.join(params.dump_path, "text.prefix")
            self.file_handler_prefix = io.open(params.export_path_prefix, mode="a", encoding="utf-8")

            if params.separate_modality:
                params.export_path_data = os.path.join(params.dump_path, "data.h5")
                total_lines = params.max_epoch * params.n_steps_per_epoch * params.batch_size
                if env.generator.space_dim == 0:
                    self.data_matrix = np.zeros(
                        (total_lines, params.t_num, params.max_output_dimension),
                        dtype=np.single,
                    )
                else:
                    self.data_matrix = np.zeros(
                        (total_lines, params.t_num, env.generator.x_grid_size, params.max_output_dimension),
                        dtype=np.single,
                    )
                self.cur_line = 0
                logger.info(f"Text will be stored in prefix in: {params.export_path_prefix} ...")
                logger.info(f"Data will be stored in h5 in: {params.export_path_data} ...")
            else:
        ...
        self.space_dim = env.generator.space_dim
        if self.space_dim > 0:
            # self.x_grid = torch.from_numpy(env.generator.x_grid.astype(np.single))
            self.x_grid_size = env.generator.x_grid_size
            # self.input_tx_grid = env.get_tx_grid(
            #     self.t_eval[0 : params.input_len], self.x_grid, self.space_dim, self.x_grid_size
            # )  # (t_num*x_num, 1+space_dim)
            # self.output_tx_grid = env.get_tx_grid(
            #     self.t_eval[params.input_len :], self.x_grid, self.space_dim, self.x_grid_size
            # ).cuda()
        else:
            # self.x_grid = None
            self.x_grid_size = 1
    ...
    def get_batch(self, task):
        ...
        try:
            ...
            logger.info("Reached end of dataloader, restart...")
    ...
    def export_data(self, task):
        ...
        for i in range(processed_e):
            ...
            if self.params.separate_modality:
                ...
                outputs["dim"] = dim
                if self.space_dim == 0:
                    cur_data = cur_data.numpy().reshape(self.params.t_num, dim)
                else:
                    cur_data = cur_data.numpy().reshape(self.params.t_num, self.x_grid_size, dim)

                self.data_matrix[self.cur_line, ..., :dim] = cur_data
                self.cur_line += 1
            else:
    ...
    def enc_dec_step(self, task):
        ...
        if "normalizer" in self.modules:
            normalizer = self.modules["normalizer"]
            normalizer.train()
        ...
        train_output_start = params.train_output_start
        assert t_eval.dtype == torch.float32

        data = samples["data"]  # (bs, t_num, x_num, dim)
        ...
        # eps = 1e-6
        # norms = [torch.linalg.norm(sample) for sample in data]
        # normalized_data = [data[i] / (norms[i] + eps) for i in range(len(data))]

        if self.space_dim == 0:
            data_input, data_label, data_len, data_dim = env.batch_data_operator(data, t_eval, params.input_step, dims)
            # normalized_data_input, normalized_data_label, _,_= env.batch_data_operator(normalized_data, t_eval, params.input_step, dims)
            # data_input size: (input_len, bs, output_dim+1)
            ...
            data_y = data_label[train_output_start :: params.train_output_step, :, :]
            data_pred_mask = data_pred_mask[None].expand_as(data_y)

            if self.params.data_loss_reweight == "none":
                loss_weight = None
            elif self.params.data_loss_reweight == "l2":
                # reweight by L2 norm squared
                loss_weight = torch.linalg.vector_norm(data_y, dim=(0, -1), keepdim=True) ** 2  # (1, bs, 1)
            elif self.params.data_loss_reweight == "linfty":
                # reweight by Linfinity norm
                loss_weight, _ = torch.max(torch.abs(data_y), dim=0, keepdim=True)  # (1, bs, output_dim)
            else:
                assert False, f"Unknown reweight type: {self.params.data_loss_reweight}"

        else:
            data_input, data_label, data_len, data_dim = env.batch_data_pde_operator(
                data, t_eval, self.x_grid_size, dims=dims, step=params.input_step
            )

            # normalized_data_input, normalized_data_label, _,_=  env.batch_data_pde_operator(
            #     normalized_data, t_eval, self.x_grid_size, dims=dims
            # )
            # data_input size: (input_len, bs, 1+output_dim*x_grid_size)
            # data_label size: (slen, bs, x_grid_size, output_dim)

            data_adim = torch.arange(params.max_output_dimension, dtype=torch.long, device=data_dim.device)
            data_pred_mask = data_adim[None] < (data_dim[:, None])  # (bs, output_dim)
            # input_tx_len = input_len * self.x_grid_size
            data_y = data_label[
                train_output_start :: params.train_output_step
            ]  # (output_len, bs, x_grid_size, output_dim)
            data_pred_mask = data_pred_mask[None, :, None, :].expand_as(data_y)

            if self.params.data_loss_reweight == "none":
                loss_weight = None
            elif self.params.data_loss_reweight == "l2":
                # reweight by L2 norm squared
                loss_weight = torch.linalg.vector_norm(data_y, dim=(0, 2, 3), keepdim=True) ** 2  # (1, bs, 1, 1)
                # print(loss_weight.shape)
                # print(loss_weight)
            elif self.params.data_loss_reweight == "linfty":
                # reweight by Linfinity norm
                loss_weight, _ = torch.max(torch.abs(data_y), dim=0, keepdim=True)  # (1, bs, 1, output_dim)
            else:
                assert False, f"Unknown reweight type: {self.params.data_loss_reweight}"
        ...
        # assert len(data_y) == data_dim.sum().item() * (params.t_num - input_len) * self.x_grid_size
        ...
        with torch.cuda.amp.autocast(enabled=(params.amp >= 0), dtype=torch.bfloat16):
            ...
            if self.params.normalization:
                data_input, mu, var = normalizer(data_input)
            ...
            if not self.params.text_only:
                # if self.space_dim == 0:
                query_emb = data_decoder(
                    "query_emb", query_times=t_eval[train_output_start :: params.train_output_step]
                )  # query locations (data_len, dim)
                # else:
                #     query_emb = data_decoder("query_emb", query_times=self.output_tx_grid)
                ...
                if self.params.normalization:
                    data_decoded = normalizer.reverse(data_decoded, mu, var)
